...
$(function() {
	...
	$('#submit_quanti').click(function() {
		...
		var method = "GM";
		if ($("select option:selected").text() == "Gambling Method") {
			method = "GM";
		} else if ($("select option:selected").text() == "Probability Wheel") {
			method = "PW";
		} 
		...
		if (!(name || unit || val_min || val_max) || isNaN(val_min) || isNaN(val_max)) {
			...
			$('#att_name_quanti').val("");
			$('#att_unit_quanti').val("");
			$('#att_method_quanti option[value="GM"]').prop('selected', true);
